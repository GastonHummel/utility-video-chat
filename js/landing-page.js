        ...
        switch (e.keyCode) {
          ...
          case 39:
            ...
            if (currentDate.getTime() > keyStart[e.keyCode].getTime() + 500) {
              if (
                VideoPlayerInterface.iframeWindow.rtc.player.vars.videoDuration !== 0 &&
                VideoPlayerInterface.iframeWindow.rtc.player.vars.videoDuration < 10
              ) {
                if (e.keyCode == 37) {
...
var SettingsPanel = {
  ...
  events: {
    ...
    settingsButtonClickEventHandler: function (e) {
      ...
      if (
        $('#jsCCSelectorPopout').is(':visible') ||
        $('#jsQualitySelectorPopout').is(':visible') ||
        $('#jsLanguageSelectorPopout').is(':visible') ||
        popout.is(':visible')
      ) {
        $('#jsCCSelectorPopout').hide();
        $('#jsQualitySelectorPopout').hide();
...
var Timeline = {
  ...
  render: function (chapters) {
    ...
    $.each(chapters, function createChapter(i, settings) {
      ...
      $.each(settings.states, function createChapterState(j, state) {
        ...
        div.data('percent-width', state.width).data('percent-start', start).data('state', state.cardId);
  ...
  events: {
    ...
    initialise: function () {
      ...
      $('#jsTimelineIndicator').mousedown(Timeline.events.timelineIndicatorMousedown);

      $(document).mouseup(Timeline.events.documentMouseup).mousemove(Timeline.events.documentMousemove);
  ...
  updateFromVideo: function () {
    try {
      if (
        ...
        VideoPlayerInterface.iframeWindow.rtc.player &&
        VideoPlayerInterface.iframeWindow.rtc.player.playersReady()
      ) {
        ...
        Timeline.updateStateAndProgress(VideoPlayerInterface.currentState, times.play / times.status.duration || 0);
...
var VolumeSlider = {
   ...
   */
  disabledDevices: [/ipad/, /iphone/, /ipod/, /android(?!.*?mobile)/, /android.*?mobile/, /blackberry/, /windows ce/, /iemobile/, /webos/, /playbook/],
...
var MobileOrientationHandler = {
  ...
  resizeVideoArea: function () {
    var mainWidth;
    if (
      $(window).trueInnerWidth() / $(window).trueInnerHeight() >= 16 / 9 &&
    ...
    } else if ($(window).trueInnerWidth() / $(window).trueInnerHeight() > 16 / 9 && MobileOrientationHandler.shouldEnable()) {
...
var FullScreenHandler = {
  ...
  events: {
     ...
     */
    initialise: function () {
      $(document).on('fullscreenchange webkitfullscreenchange mozfullscreenchange MSFullscreenChange', FullScreenHandler.events.toggleHtmlChanges);
