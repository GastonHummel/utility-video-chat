...
var ConverseUtil = {
  ...
  isValidUrl: function (msg) {
    return msg.match(/^(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\%\]@!\$&'\(\)\*\+,;=.]+$/gi);
  },
...
ConverseWebClient.prototype._init = function (callback) {
  var client = this,
    ...
    queries = ['tenant=' + encodeURIComponent(client.tenant), 'bot_id=' + encodeURIComponent(client.botId)];
...
ConverseWebClient.prototype._buildInterface = function () {
    ...
    $btnGroup.append(
      $(
        '<button data-widget="minimize" ' +
          ...
          'type="button">' +
          '<i class="pb-chat-minimize-icon fas ' +
          client.minimizeIcon +
          '"></i>' +
  ...
  var $messageBox = $('<div />').addClass('pb-chat-popup-messages-message-box').append($sendField);
...
ConverseWebClient.prototype.messageSend = function () {
  ...
  if (files && files.length) {
    ...
    setTimeout(function () {
      ...
      $(_this.baseContainer)
        .find(_this.baseContainer + '-pb-file-upload')
        .val('');
      $(_this.baseContainer)
        .find(_this.baseContainer + '-pb-file-name')
        .text('');
    }, fileTimeoutValue);
  }
};
...
ConverseWebClient.prototype.renderMain = function (callback, history) {
  ...
  $(me.baseContainer)
    ...
    .keydown(function (event) {
      ...
      if (event.which === 8 || event.which === 32 || (event.which > 44 && event.which < 112) || event.which > 123) {
        // alt, ctrl, esc, F1, F2, ...
        me.userIsTyping();
      }
    });
  ...
  $(me.baseContainer)
    ...
    .click(function onClickHeader(event) {
  ...
  $(me.baseContainer).find('#pb-dropdown').onclick = function (event) {
    if (event.target && event.target.offsetParent && event.target.id !== 'pb-dropdown' && event.target.offsetParent.id !== 'pb-dropdown') {
  ...
  if (this.showToolbar) {
    $(me.baseContainer)
      ...
      .change(function () {
        ...
        $(me.baseContainer)
          .find(me.baseContainer + '-pb-file-name')
          .text(fileString);
        $(me.baseContainer)
          .find(me.baseContainer + '-pb-file-name')
          .attr('title', fileString);
      });

    $(me.baseContainer)
      .find(me.baseContainer + '-pb-file-upload')
      .on('click', function () {
        $(me.baseContainer)
          .find(me.baseContainer + '-pb-file-upload')
          .val('');
...
ConverseWebClient.prototype.setWrapperItems = function (data) {
  ...
  if (this.lastTimestamp < new Date().getTime() - this.REDATE_TIME * (1000 * 60)) {
    ...
    $(this.baseContainer)
      .find('#messages')
      .append($('<div class="pb-chat-box-single-line"><abbr class="timestamp">' + this.getTimeStamp() + '</abbr></div>'));
  }
...
ConverseWebClient.prototype.addDownloadMessage = function (data, options) {
  ...
  if (data.type.indexOf('image') === -1) {
    $messageBodyDiv.append(
      $(
        "<div class='pb-download-name'>" +
          data.originalName +
          "</div><a target='_new' class='pb-download-link' href='" +
          this.converseHost +
          '/' +
          data.link +
          "' alt='" +
...
ConverseWebClient.prototype.addChatMessage = function (data, options) {
  ...
  if (data.message.trim().substring(0, 29) === 'https://www.youtube.com/embed' && data.direction === 'inbound') {
    ...
    $messageBodyDiv = $('<img class="pb-chat-direct-chat-text-' + data.direction + ' ">').src(data.message);
  } else if (this.allowHTML) {
    $messageBodyDiv = $('<div class="pb-chat-direct-chat-text-' + data.direction + ' ">').html(data.message);
  } else {
    $messageBodyDiv = $('<div class="pb-chat-direct-chat-text-' + data.direction + ' ">').text(data.message);
  }
  this.addMessageElement($messageBodyDiv, data);
};
...
ConverseWebClient.prototype.disableTextArea = function () {
  $(this.baseContainer).find('textarea').attr('placeholder', this.disabledPlaceholder).prop('disabled', true);
...
ConverseWebClient.prototype.enableTextArea = function () {
  $(this.baseContainer).find('textarea').attr('placeholder', this.placeholder).prop('disabled', false);
